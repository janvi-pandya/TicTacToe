...
it('Should indicate O as a winner in leftDiagonal', () => {
  const board = {
    0: [O, X, X],
    1: ['', O, ''],
    2: [X, '', O]
  ...
  expect(xResult.won).toBe(false);
  expect(oResult.won).toBe(true);
  expect(oResult.line).toBe('leftDiagonal');
});

it('Should indicate X as a winner in rightDiagonal', () => {
  const board = {
    0: ['', O, X],
    1: ['', X, ''],
    2: [X, '', O]
  ...
  const oResult = resultForSymbol(O, board);
  expect(xResult.won).toBe(true);
  expect(xResult.line).toBe('rightDiagonal');
  expect(oResult.won).toBe(false);
});